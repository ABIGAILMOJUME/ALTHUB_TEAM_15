...
def forgot_password(request: Request,payload: Annotated[ForgotPassword, Form()], db: Session = Depends(get_db)):
    ...
    logger.info(f"Password reset link for {user.email}")
...
def reset_password(request: Request,requests: Annotated[ResetPassword, Form()], db: Session = Depends (get_db)):
    payload = decode_token(requests.token)
    ...
    user.hashed_password = hash_password(requests.new_password)
    db.commit()
    db.refresh(user)
